package pv.alg.gurobimodelmintime;
...
import pv.alg.gurobimodel.GurobiHelperFunctions;
...
public class LpTurnusFuzzyMinimizeTransitTime {
    ...
    public LpTurnusFuzzyMinimizeTransitTime createModel(List<Spoj2> possibleConnections, Map<Integer,Map<Integer, Triple<Long, Long, Long>>> dist, int busCount, ModelMode modelMode) {
    ...
    private void createConBusCount() throws GRBException {
        ...
        model.addConstr(cons, GRB.EQUAL, busCount, "c" + c_forName++);
    }
    ...
    private void createObjFunction() throws GRBException {
        ...
        spoje2.forEach(s -> {
            s.getPossibleConnectionsFromThis().forEach(n -> {
                ...
                if (this.modelMode == ModelMode.OPTIMISTIC) {
                    coef[index.getAndIncrement()] = distances.get(s.getToId()).get(n.getFromId()).getLeft();
                } else {
                    coef[index.getAndIncrement()] = distances.get(s.getToId()).get(n.getFromId()).getRight();
                }
